...
def test_create_book():
    new_book_data = {"title": "The lord of the rings", "year": "1950", "authors": []}
    new_book_data_json = json.dumps(new_book_data)
    resp: httpx = httpx.post(f"{BASE_URL}/books", data=new_book_data_json)
    assert resp.status_code == 201


def test_create_author():
    new_author_data = {"name": "J.R.R Tolkien", "country": "England", "books": []}
    new_author_data_json = json.dumps(new_author_data)
    resp: httpx = httpx.post(f"{BASE_URL}/authors", data=new_author_data_json)
    assert resp.status_code == 201
...
def test_relation():
    ...
    assert authors[0] == {"id": 1, "name": "J.R.R Tolkien", "country": "England"}
